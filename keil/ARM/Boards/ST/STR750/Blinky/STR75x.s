...
;//   <h> Clock Control Register (MRCC_CLKCTL)
...
;// </e>
MRCC_SETUP      EQU     1
MRCC_CLKCTL_Val EQU     0x03000008
...
; MCU Reset and Clock Control --------------------------------------------------
                ...
                BIC     R2, R2, #NCKDF_BIT
WAIT_OSC_PRES   STR     R2, [R0, #MRCC_CLKCTL_OFS]  ; Write 1 to NCKDF Flag
                LDR     R1, [R0, #MRCC_CLKCTL_OFS]  ; Read       NCKDF Flag
                ANDS    R1, R1, #NCKDF_BIT
                BNE     WAIT_OSC_PRES				; if NCKDF Flag set to 0
...
WAIT_CLK_STABIL LDR     R1, [R0, #MRCC_RFSR_OFS]    ; Wait stabil clock
                ...
                LDR     R1, [R0, #MRCC_CLKCTL_OFS]  ; Load PLL Mul Settings
                LDR     R3, =MUL_MASK
                LDR     R4, =0xFFFFFFFF
                EOR     R4, R3, R4
                AND     R1, R1, R4
                LDR     R2, =MRCC_CLKCTL_Val
                AND     R2, R2, R3
                ORR     R1, R1, R2
                ORR     R1, R1, #CKSEL_BIT
                STR     R1, [R0, #MRCC_CLKCTL_OFS]
...
PLL_LOCKING     LDR     R1, [R0, #MRCC_CLKCTL_OFS]  ; Wait for PLL to lock
                ...
                LDR     R1, =MRCC_CLKCTL_Val		; Load all Clock Settings
                STR     R1, [R0, #MRCC_CLKCTL_OFS]
                ENDIF	; ------------------------------------------------------
