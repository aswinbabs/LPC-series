...
void USB_Reset_Event (void) {
  USB_ResetCore();
  AT91D_BASE_PIO_LED->PIO_SODR = AT91B_LED3; /* Turn Off LED3 */
  AT91D_BASE_PIO_LED->PIO_SODR = AT91B_LED4; /* Turn Off LED4 */
}
#endif
...
void USB_Suspend_Event (void) {
  AT91D_BASE_PIO_LED->PIO_CODR = AT91B_LED4; /* Turn On LED4 */
}
#endif
...
void USB_Configure_Event (void) {
  ...
  if (USB_Configuration) {                  /* Check if USB is configured */
    GetMouseInReport();
    USB_WriteEP(0x83, MouseInReport, sizeof(MouseInReport));
    AT91D_BASE_PIO_LED->PIO_CODR = AT91B_LED3; /* Turn On LED3 */
  } else {
    AT91D_BASE_PIO_LED->PIO_SODR = AT91B_LED3; /* Turn Off LED3 */
  }
}
