...
BOOL MSC_Reset (void) {
  ...
  AT91D_BASE_PIO_LED->PIO_SODR = AT91B_LED1 | AT91B_LED2; /* Turn Off R/W LED */
...
void MSC_MemoryRead (void) {
  ...
  if (BulkStage != MSC_BS_DATA_IN) {
    AT91D_BASE_PIO_LED->PIO_SODR = AT91B_LED1; /* Turn Off Read LED */
...
void MSC_MemoryWrite (void) {
  ...
  if ((Length == 0) || (BulkStage == MSC_BS_CSW)) {
    AT91D_BASE_PIO_LED->PIO_SODR = AT91B_LED2; /* Turn Off Write LED */
    CSW.bStatus = CSW_CMD_PASSED;
    MSC_SetCSW();
  }
}
...
fail: CSW.bStatus = CSW_CMD_FAILED;
      ...
      switch (CBW.CB[0]) {
        ...
        case SCSI_READ10:
          if (MSC_RWSetup()) {
            if ((CBW.bmFlags & 0x80) != 0) {
              AT91D_BASE_PIO_LED->PIO_CODR = AT91B_LED1; /* Turn On Read LED */
        ...
        case SCSI_WRITE10:
          if (MSC_RWSetup()) {
            if ((CBW.bmFlags & 0x80) == 0) {
              AT91D_BASE_PIO_LED->PIO_CODR = AT91B_LED2; /* Turn On Write LED */
