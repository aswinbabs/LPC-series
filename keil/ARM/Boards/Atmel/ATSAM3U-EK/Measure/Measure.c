...
void adc_Init (void) {
  ...
  *AT91C_PMC_PCER  |= ((1UL << AT91C_ID_PIOB)  |          /* enable clock for PIOB             */
                       (1UL << AT91C_ID_ADCC0) |          /* enable clock for ADC0             */
                       (1UL << AT91C_ID_ADCC1)  );        /* enable clock for ADC1             */ 
                       
  *AT91C_PIOB_IDR = 
  *AT91C_PIOB_ODR = 
  *AT91C_PIOB_PER = (AT91C_PIO_PB5 | AT91C_PIO_PB4);      /* configure PB4, PB4 as input       */

  *AT91C_ADC0_CR   = AT91C_ADC_SWRST;                           /* SW reset */

  *AT91C_ADC0_MR   = (AT91C_ADC_TRGEN_DIS         |             /* starting conversion by SW */
                      AT91C_ADC_LOWRES            |             /* 10 bit resolution */
  ...
  *AT91C_ADC0_ACR  = 0;                                         /*                  */ 
  *AT91C_ADC0_CHER = AT91C_ADC_CH3;                             /* enable channel 3 */ 
   data = *AT91C_ADC0_CDR3 & 0x3FF;                             /* AD value (10 bit) */

  *AT91C_ADC0_CR   = AT91C_ADC_START;                           /* Start conversion */ 

  *AT91C_ADC1_CR   = AT91C_ADC_SWRST;                           /* SW reset */

  *AT91C_ADC1_MR   = (AT91C_ADC_TRGEN_DIS         |             /* starting conversion by SW */
                      AT91C_ADC_SLEEP_NORMAL_MODE |
                      (prescal <<  8)             |             /* prescaler */
                      (startup << 16)             |             /* startup time */
                      (shtim   << 24)                           /* SHTIM */
  ...
  *AT91C_ADC1_CHER = AT91C_ADC_CH0;                             /* enable channel 0 */ 
   data = *AT91C_ADC1_CDR0 & 0x3FF;                             /* AD value (10 bit) */
  *AT91C_ADC1_CR   = AT91C_ADC_START;                           /* Start conversion */ 
}
...
void io_Init (void) {
  ...
  *AT91C_PMC_PCER  |= ((1UL << AT91C_ID_PIOA) |           /* enable clock for PIOA             */
                       (1UL << AT91C_ID_PIOB)  );         /* enable clock for PIOB             */  
...
void SysTick_Handler (void) {
    ...
    if (current.time.msec == 0) {
      *AT91C_PIOB_CODR = AT91C_PIO_PB0;        /* Turn LED#1 On               */
    }
    if (current.time.msec == 100) {
      *AT91C_PIOB_SODR = AT91C_PIO_PB0;        /* Turn LED#1 Off              */
    }
  
    if (measurement_interval || mdisplay)  {   /* process measurement         */
      current.gpioa = *AT91C_PIOA_PDSR &       /* PB-LEFT, PB-RIGHT status    */
                      (AT91C_PIO_PA19 | AT91C_PIO_PA18);

      if (*AT91C_ADC0_SR & AT91C_ADC_EOC3) {
        current.analog[0] = *AT91C_ADC0_CDR3 & 0x3FF;    /* AD value (10 bit) */
        *AT91C_ADC0_CR    = AT91C_ADC_START;             /* Start conversion  */ 
      }
      if (*AT91C_ADC1_SR & AT91C_ADC_EOC0) {
        current.analog[1] = *AT91C_ADC1_CDR0 & 0x3FF;    /* AD value (10 bit) */
        *AT91C_ADC1_CR    = AT91C_ADC_START;             /* Start conversion  */ 
      }
