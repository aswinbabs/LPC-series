...
int main (void) {                       /* Main Program                       */
  ...
  AT91C_BASE_PIOB->PIO_IDR  =           /* Setup Pins PB0..PB1 for LEDs       */
  AT91C_BASE_PIOB->PIO_OER  =
  AT91C_BASE_PIOB->PIO_PER  = (AT91C_PIO_PB1 | AT91C_PIO_PB0);
  ...
  for (;;) {                            /* Loop forever                       */
    ...
    if (ClockLEDOn) {    /* Blink LED every 1 second (for 0.5s)*/
      ...
      AT91C_BASE_PIOB->PIO_CODR = led_mask[num];
    }
    if (ClockLEDOff) {
      ClockLEDOff = 0;
      AT91C_BASE_PIOB->PIO_SODR = led_mask[num];
    }
  }
}
