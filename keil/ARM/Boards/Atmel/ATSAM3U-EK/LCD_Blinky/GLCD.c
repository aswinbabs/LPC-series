...
#define LCD_D15         (AT91C_PB6_D15)
#define LCD_RS          (AT91C_PB8_A1)
#define LCD_RD          (AT91C_PB19_NRD)
#define LCD_WR          (AT91C_PB23_NWR0_NWE)
#define LCD_NCS2        (AT91C_PC16_NCS2)
#define LCD_BACKLIGHT   (AT91C_PIO_PC19)

void GLCD_init (void) { 
  unsigned int mode, i;

/* Configure the LCD Control pins --------------------------------------------*/
  AT91C_BASE_PMC->PMC_PCER = ((1 << AT91C_ID_PIOB) |      /* enable clock for PIOB */
                              (1 << AT91C_ID_PIOC) );     /* enable clock for PIOC */

/* Configure the Data Bus pins -----------------------------------------------*/
   AT91C_BASE_PIOB->PIO_PDR    =  (LCD_D0_D14 | LCD_D15); /* enable Peripheral function */  
   AT91C_BASE_PIOB->PIO_IDR    =  (LCD_D0_D14 | LCD_D15); /* disable interrupt */ 
   AT91C_BASE_PIOB->PIO_ABSR  &= ~(LCD_D0_D14);           /* select Peripheral A */ 
   AT91C_BASE_PIOB->PIO_ABSR  |=  (LCD_D15);              /* select Peripheral B */ 

/* Configure the Control pins -----------------------------------------------*/
   AT91C_BASE_PIOB->PIO_PDR    =  (LCD_RS);               /* enable Peripheral function */ 
   AT91C_BASE_PIOB->PIO_IDR    =  (LCD_RS);               /* disable interrupt */ 
   AT91C_BASE_PIOB->PIO_ABSR  |=  (LCD_RS);               /* select Peripheral B */ 

   AT91C_BASE_PIOB->PIO_PDR    =  (LCD_RD | LCD_WR);      /* enable Peripheral function */ 
   AT91C_BASE_PIOB->PIO_IDR    =  (LCD_RD | LCD_WR);      /* disable interrupt */ 
   AT91C_BASE_PIOB->PIO_ABSR  &= ~(LCD_RD | LCD_WR);      /* select Peripheral A */ 

   AT91C_BASE_PIOC->PIO_PDR    =  (LCD_NCS2);             /* enable Peripheral function */  
   AT91C_BASE_PIOC->PIO_IDR    =  (LCD_NCS2);             /* disable interrupt */ 
   AT91C_BASE_PIOC->PIO_ABSR  &= ~(LCD_NCS2);             /* select Peripheral A */ 

/* Configure the Backlight pins --------------------------------------------*/
  AT91C_BASE_PIOC->PIO_IDR  =                             /* disable interrupt */
  AT91C_BASE_PIOC->PIO_OER  =                             /* enable output */
  AT91C_BASE_PIOC->PIO_PER  = (LCD_BACKLIGHT);            /* disable Peripheral function */ 
  AT91C_BASE_PIOC->PIO_CODR = (LCD_BACKLIGHT);            /* Backlight off */

/*-- SMC Configuration -------------------------------------------------------*/
/*--------------------------- CS2 --------------------------------------------*/
  AT91C_BASE_PMC->PMC_PCER = (1 << AT91C_ID_HSMC4);       /* enable clock for SMC   */

  AT91C_BASE_HSMC4_CS2->HSMC4_SETUP = 0
  ...
  AT91C_BASE_HSMC4_CS2->HSMC4_PULSE = 0
   ...
   AT91C_BASE_HSMC4_CS2->HSMC4_CYCLE = 0
                  | ((22 <<  0) & AT91C_HSMC4_NWE_CYCLE)
                  | ((22 << 16) & AT91C_HSMC4_NRD_CYCLE)
                  ;

  mode = AT91C_BASE_HSMC4_CS2->HSMC4_MODE;
  AT91C_BASE_HSMC4_CS2->HSMC4_MODE = (mode & ~(AT91C_HSMC4_DBW | AT91C_HSMC4_READ_MODE
  ...
  for (i = 0; i < 25 /* level */; i++) {
    AT91C_BASE_PIOC->PIO_SODR = (LCD_BACKLIGHT);
    AT91C_BASE_PIOC->PIO_SODR = (LCD_BACKLIGHT);
    AT91C_BASE_PIOC->PIO_SODR = (LCD_BACKLIGHT);

    AT91C_BASE_PIOC->PIO_CODR = (LCD_BACKLIGHT);
    AT91C_BASE_PIOC->PIO_CODR = (LCD_BACKLIGHT);
    AT91C_BASE_PIOC->PIO_CODR = (LCD_BACKLIGHT);
  }
  AT91C_BASE_PIOC->PIO_SODR = (LCD_BACKLIGHT);
